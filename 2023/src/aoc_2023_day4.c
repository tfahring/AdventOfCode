...
day4_part1(MemoryArena *arena) {
    ...
    u64 sum = 0;
    u64 *winningNumbers = NULL;
    u64 *myNumbers      = NULL;
    for (String8ListNode *line = lines.first; line; line = line->next) {
        String8List card = string8_split(arena, line->string, cardSplitChar, 2);
        ...
        String8List winningNumbersStringList = string8_split(arena, winningNumbersString, numSplitChar, 1);
        String8List myNumbersStringList      = string8_split(arena, myNumbersString, numSplitChar, 1);

        if (winningNumbers == NULL) {winningNumbers = mem_arena_push(arena, winningNumbersStringList.nodeCount, u64);}
        if (myNumbers == NULL)      {myNumbers      = mem_arena_push(arena, myNumbersStringList.nodeCount, u64);}
